  ...
  return (
    <main className="px-5 pb-24 pt-32">
      <section className="mx-auto max-w-5xl">
        <p className="text-xs font-black uppercase text-brand-coral">Blog</p>
        <h1 className="mt-4 max-w-4xl font-display text-5xl font-black uppercase leading-none tracking-normal sm:text-7xl">Learning notes and engineering logs.</h1>
        ...
        <div className="mt-12 grid gap-5">
          {blogPosts.map((post) => (
            <article key={post.slug} className="panel rounded-feature p-6 transition hover:-translate-y-1">
              <p className="text-xs font-black uppercase text-brand-coral">{post.date}</p>
              <h2 className="mt-3 font-display text-3xl font-black uppercase tracking-normal">{post.title}</h2>
