  ...
  return (
    <main className="px-5 pb-24 pt-32">
      <article className="mx-auto max-w-3xl">
        <Link href="/blog" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-black text-slate-300 hover:bg-white/10">
          <ArrowLeft size={16} /> Back to blog
        </Link>

        <p className="mt-10 font-mono text-xs font-black uppercase tracking-[0.16em] text-cyan">{post.date}</p>
        <h1 className="mt-4 text-5xl font-black leading-none tracking-[-0.07em] sm:text-7xl">{post.title}</h1>
        <p className="mt-6 text-lg leading-8 text-slate-400">{post.excerpt}</p>

        <div className="mdx-body mt-12 rounded-card border border-white/10 bg-white/5 p-7 sm:p-10">
          <h2>Context</h2>
